...
if (hour < 0 || hour > 23) {
  ...
  // variable.
  if (street == "ash") {
    isParkingAllowed = CanParkOnAsh(day, hour);
  } else if (street == "beech") {
    isParkingAllowed = CanParkOnBeech(day, hour);
  } else if (street == "cedar") {
    isParkingAllowed = CanParkOnCedar(day, hour);
  } else if (street == "date") {
    isParkingAllowed = CanParkOnDate(day, hour, minute);
  } else if (street == "elm") {
    isParkingAllowed = CanParkOnElm(day, hour);
  }
